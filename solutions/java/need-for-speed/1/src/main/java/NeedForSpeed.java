class NeedForSpeed {
    int speed;
    int batteryDrain;
    int batteryPercentage; // Cars start with full battery
    int distance; // Distance driven by the car

    NeedForSpeed(int speed, int batteryDrain) {
        this.speed = speed;
        this.batteryDrain = batteryDrain;
        this.batteryPercentage = 100; // Initialize battery percentage to 100%
        this.distance = 0; // Initialize distance to 0
    }

    public boolean batteryDrained() {
        if (this.batteryDrain <= 0) {
            return true; // Battery is drained if the drain percentage is zero or less
        }
        return false; // Battery is not drained if the drain percentage is positive
    }

    public int distanceDriven() {
        return this.distance; // Return the distance driven by the car
    }

    public void drive() {
        if (this.batteryDrained()) {
            throw new UnsupportedOperationException("Cannot drive, battery is drained");
        }
        if (this.batteryPercentage < this.batteryDrain) {
            throw new UnsupportedOperationException("Cannot drive, not enough battery");
        }
    ...
    public static NeedForSpeed nitro() {
        int speed = 50; // Nitro car speed
        int batteryDrain = 4; // Nitro car battery drain
        return new NeedForSpeed(speed, batteryDrain); // Return a new instance of NeedForSpeed for Nitro car
    }
}

class RaceTrack {
    int distance; // Distance of the
    ...
    public boolean canFinishRace(NeedForSpeed car) {
        int totalBatteryDrain = (this.distance / car.speed) * car.batteryDrain; // Calculate total battery drain for the
        return car.batteryPercentage >= totalBatteryDrain; // Check if the car
    }
}
